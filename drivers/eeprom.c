...
void eeprom_cs_init(void)
{
	...
	rslt = cyhal_gpio_init(PIN_SPI_EEPROM_CS, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, 0);
...
void eeprom_write_byte(uint16_t address, uint8_t data)
{
	...
	uint8_t transmit_data[4] 	= {EEPROM_CMD_WRITE, highByte,lowByte,data};
...
uint8_t eeprom_read_byte(uint16_t address)
{
	...
	int highByte = (address >> 8) & 0xFF;
    int lowByte = address & 0xFF;
	uint8_t transmit_data[4] 	= {EEPROM_CMD_READ, highByte,lowByte,0};
	uint8_t receive_data[4] 	= {0,0,0,0};
	cy_rslt_t rslt;
