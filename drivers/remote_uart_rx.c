...
bool remote_uart_rx_data_async(char *msg, uint16_t max_size)
{
    ...
    // while(!circular_buffer_empty(Rx_Circular_Buffer)){
    //     if(max_size==size) break;
    //     *msg = circular_buffer_remove(Rx_Circular_Buffer);
    //     msg++;
    //     size++;
    // }
    for(int i=0;i<max_size;i++){
        if(circular_buffer_empty(Rx_Circular_Buffer)==true) break;
        *msg=circular_buffer_remove(Rx_Circular_Buffer);
        msg++;
    }
...
void remote_uart_event_handler_process_rx(void)
{
    ...
    if(status!=CY_RSLT_SUCCESS) return;
